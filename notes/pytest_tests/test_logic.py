...
def test_user_can_create_note(author_client, author, form_data):
    ...
    # Чтобы проверить значение полей заметки -
...
# Добавляе маркер, который обеспечит доступ к базе данных:
...
# Вызываем фикстуру отдельной замтеки, чтобы в базе появилась запись.
def test_not_unique_slug(author_client, note, form_data):
    """Проверяю, что невозможно создать две заметки с одинаковым slug."""
    url = reverse('notes:add')
    # Поменяем slug новой заметки на slug уже существующей записи:
...
def test_author_can_edit_note(author_client, form_data, note, slug_for_args):
    ...
    # Получаем адрес страници редактирования заметки:
...
def test_other_user_cant_edit_note(not_author_client, form_data, note,
    ...
    # Проверяем, что странимца не найдена:
    assert response.status_code == HTTPStatus.NOT_FOUND
    # Получаем новый объект запросом из БД.
    note_from_db = Note.objects.get(id=note.id)
    # Проверяем, что атрибуты объекта из БД атрибуам заметки до запроса.
